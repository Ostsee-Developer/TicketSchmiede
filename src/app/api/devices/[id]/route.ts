  ...
  try {
    ...
    const device = await prisma.device.findUnique({
      ...
      include: {
        ...
        software: { include: { software: { select: { id: true, name: true, vendor: true } } } },
        credentials: { select: { id: true, name: true, username: true, category: true } },
      },
    });
...
export async function PATCH(
  ...
  try {
    ...
    const updated = await prisma.device.update({
      ...
      data: {
        ...parsed.data,
        purchaseDate: parsed.data.purchaseDate ? new Date(parsed.data.purchaseDate) : parsed.data.purchaseDate,
        warrantyUntil: parsed.data.warrantyUntil ? new Date(parsed.data.warrantyUntil) : parsed.data.warrantyUntil,
      },
    });
